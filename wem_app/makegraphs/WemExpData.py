...
class WemExpData:
    """
    A class to store and access to experiment data.
    ...
    all_data : dict[int, dict[int, list]]
        Dictionary to store all data, where keys are trial numbers and values are dictionaries of generation data.
    trials_gens_unique_words : dict[int, dict[int, set]]
        Dictionary to store unique words for each trial and generation.
    trials_gens_count_words : dict[int, dict[int, Counter]]
        Dictionary to store word counts for each trial and generation.
    """
