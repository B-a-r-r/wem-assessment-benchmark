...
class ExpSpatialAnalyzer(WemVisualsMaker):
    ...
    def _calculate_contour_areas(self,
        ...
        -----------
        trial : int, default=-1
            Trial index for which to calculate areas
            If -1, uses all trials combined
    ...
    def _calculate_density_statistics(self, only_trials: list[int] =None) -> dict:
        ...
        -----------
        trial : int
            Trial index for which to calculate statistics
            If -1, uses all trials combined
    ...
    def _return_right_coords(self, only_trials: list[int] =None) -> np.ndarray:
        """
        Return the coordinates for the specified trial
        
        Parameters
        -----------
        trial : int
            Trial index for which to return coordinates
            If -1, returns coordinates for all trials combined
    ...
    def _calculate_unique_points(self, only_trials: list[int] =None, tolerance: float =None):
        ...
        -----------
        trial : int
            Trial index for which to calculate unique points
            If -1, uses all trials combined
    ...
    def _calculate_exploration_metrics(self, only_trials: list[int] =None, coverage_percentile: int =95, tolerance: float =None):
        ...
        -----------
        trial : int
            Trial index for which to calculate metrics
            If -1, uses all trials combined
